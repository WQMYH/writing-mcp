...
try {
  ...
  const exploreLatencies = [], contextLatencies = [], contexts = [], estimators = new Set();
  for (let run = 0; run < measuredRunsPerTask; run++) for (const task of tasks.explore) { const started = performance.now(); await service.explore(resolved.workRef, task.operation ?? "search", task.query ?? "", task.limit ?? 20, task.maxHops ?? 2, task.targetChapter); exploreLatencies.push(performance.now() - started); }
  for (let run = 0; run < measuredRunsPerTask; run++) for (const task of tasks.context) { const started = performance.now(), packet = await service.context(resolved.workRef, task.query ?? "", task.budgetTokens ?? 1000, task.requiredRefs ?? [], task.options ?? {}); contextLatencies.push(performance.now() - started); if (run === 0) { const packetText = JSON.stringify(packet), accountedInputs = packet.blocks.map((block) => ({ ref: block.ref, text: block.evidence.excerpt, estimatedTokens: block.tokens })), accountedTokens = accountedInputs.reduce((sum, input) => sum + input.estimatedTokens, 0); if (accountedTokens !== packet.usedTokens) throw new Error(`Context token accounting mismatch: blocks=${accountedTokens}, packet=${packet.usedTokens}`); estimators.add(packet.estimator); contexts.push({ query: task.query ?? "", packet: { ...packet, refs: packet.blocks.map((block) => block.ref), serialization: { text: packetText, chars: packetText.length, utf8Bytes: Buffer.byteLength(packetText, "utf8"), sha256: sha256(packetText) } }, accountedInputs, estimator: packet.estimator, usedTokens: packet.usedTokens, accountingScope: "evidence_excerpts_only", refs: packet.blocks.map((block) => block.ref) }); } }
  ...
  const report = { schemaVersion: 2, corpus: basename(corpus), parsedChars, measurement: { warmupRunsPerTask: 1, measuredRunsPerTask, percentile: 0.95 }, index: { elapsedMs: indexMs, normalizedMsPerMillionChars: indexPerMillionMs }, explore: { count: exploreLatencies.length, p95Ms: exploreP95Ms }, context: { count: contextLatencies.length, p95Ms: contextP95Ms }, gates: { maxIndexPerMillionMs, maxExploreP95Ms, maxContextP95Ms }, accountingScope: "evidence_excerpts_only", externalTokenResult: "not_evaluated" };
