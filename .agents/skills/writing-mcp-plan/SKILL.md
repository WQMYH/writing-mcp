...
本 Skill 由 `plan-to-skill` 元技能（v0.3.0）生成：先经步骤 0 计划审查（状态唯一、文件边界合格），再按生成规则产出。它是 `Writing_MCP_Server_v2.md` 执行计划的**操作手册**：在哪读什么、恢复时做什么、按什么顺序修、何时提交。不是契约本身——契约细节以各文档为准。
...
- 本 skill 由 `plan-to-skill` v0.3.0 生成；计划结构变化时按元技能步骤 0 重新审查并再生成，不手工改本文件维持。
