...
当前冻结实现中，`mentions` 是 schema v4 已有、现由 M0 词汇表显式公开的关系：原生 `[[alias]]` 仅在持久化别名有唯一 canonical owner 时确定性解析为 `Document → Entity`，逐边保留 alias capture 的 span、定位与 revision 证据；多 owner 别名进入未解析候选而不伪装为确定事实。它不能替代独立的 `Entity → Document` `appears_in` 关系。该对齐不新增源文件写入或 Agent 判断。
