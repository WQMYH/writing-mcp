...
>
> 检查点时间：2026-08-31（HB-M0～HB-M4 已交付；HB-M5 联合启动与开发体验尚未开始）
> 当前状态：可靠性 Task 1～7 已完成并完成整分支复审，未发现未关闭的 Critical/Important；PRF 生产配置为 `topK=12/termCount=8/weight=0.35`。代码检查点 `75a537b` 的完整内容通过 `pnpm verify` 与 `pnpm verify:private`：41 个测试文件、196 项测试、lint 0、公共基准 30/30、coverage lines 95.01%；私有 top-20 为 41/42、required 16/16、证据覆盖 100%，黄金 train/holdout 无回退。4,789,903 字符《语料B》门禁为索引 11.92s/百万字符、暖 Explore P95 4.42ms、暖 Context P95 4.76ms；生产搜索使用 revision-scoped 有界暖缓存，评测入口不使用该缓存。外部 tokenizer 仍为 `not_evaluated`，M4 不得据启发式估算宣称精确 Token 收益。隐私门禁已建立并完成首轮清洗：仓库历史经第二次重写把提交元数据里的个人邮箱统一为 GitHub noreply（与首轮清除私有语料书名与硬编码路径同规格：全部提交 ID 变更，本文档引用已重映射），`privacy:gate`（history scope）纳入 `pnpm verify` 链首；worktree 128 文件 / history 697 对象扫描 0 命中。远端历史对齐已完成：`main` 先经用户显式确认后强推至 `09442fa`，随后 `f5fed02` 与 `b8c7428` 已 fast-forward 推送；其后的 `2019ed3`（AGPL-3.0-only 重许可）已在 origin。`9048b21`（HB-M0）为**本地提交，尚未推送**——按 2026-08-31 用户确立的纪律，一切 push 只在用户明确指令下执行。**HB-M0 已交付（`9048b21`）**：新增 `@writing-mcp/host-bridge-protocol`（strict Zod 协议 v1、四状态模型、13 个冻结错误码、限额/超时/配对常量、鉴权矩阵数据、确定性 `computeProjectKey`/`computeSnapshotHash`/`computeContentHash`）与 `@writing-mcp/host-plugin-storyforge`（七字段 manifest 冻结），以及实现无关的规范 fixture 生成器 `scripts/host-bridge-fixtures.mjs` 与 `fixtures/host-bridge-protocol/`（跨仓库消费的单一真相源）；新增 17 项先失败测试（45 文件/220 项）；`docs/host-bridge/` 的 DESIGN/PROTOCOL/PLAN 为本地治理文档，不进入 Git（2026-08-31 用户确立：设计/计划类文档不入库，可执行契约=仓库内 schema+fixture）。门禁：check 0 错、lint 0 警告、基准 30/30、coverage 阈值过（lines 90.69%）；全量测试在暂存本机未提交 README.md 编辑的探针下 45 文件/220 项全绿，工作树上 `client-setup-docs.test.ts` 的唯一失败由该 README 编辑导致（与本提交无关，已两次 stash 探针证明）。HB-M0 仅为协议/治理冻结，**不宣称 Bridge 可运行**；HB-M1 安全 loopback companion 已交付（`2148329`：pairing code 单次使用/轮换/TTL、tab-local Bearer 仅存哈希、四态状态投影、死 pid+内容一致才可清理的单实例锁、有界 stderr 的 MCP stdio 客户端、loopback→Host→Origin→Bearer 顺序强制与 PNA 预检的 HTTP 边界、stderr-only 配对码 CLI 与 SIGINT/SIGTERM/stdin EOF 幂等关机；28 项先失败测试，全链 `pnpm verify` 通过：48 文件/248 测试、coverage Stmts 89.09/Branch 79.76/Funcs 88.1/Lines 92.9 均过阈值；HB-M1 收口能力已验证：CLI 启动→两次独立配对/撤销路径、MCP 子进程异常死亡转 degraded 不假 ready、stdin EOF 后锁释放无僵尸进程）。HB-M2 快照事务已交付（Writing MCP 1f70da6 与 Storyforge 330d980 配对记录）：插件注册表（七字段 manifest 校验 + enabled/disabled/revoked 持久化于全局 plugin-state.json）、每项目互斥的快照事务（严格校验/重算哈希、16 MiB/64 MiB/4096 限额、staging 写入、Windows 两阶段替换 EBUSY 有界重试、resolve→index 分支到 fresh、binding manifest 原子最后写、同快照 no-op、失败回滚、恢复失败才 degraded、重启装载为 stale 候选）、Bearer 门禁的 status/snapshot/derived-data 路由（72 MiB 预拒+流式计数、DERIVED_DATA_BUSY 守卫）；Storyforge 侧 buildSnapshotDraft（单 Dexie 只读事务 + 确定性渲染 + n:<id> 章节映射 + 敏感表负清单，6 项 fake-indexeddb 测试）。HB-M3 前复审修复 `229afea` 已闭合两个计划偏差：MCP stderr 现在保留真正最新的有界尾部，快照流程的所有 `writing_index` 调用显式使用冻结的 120 秒超时。全链 `pnpm verify` 51 文件/268 测试、公共基准 30/30、coverage lines 91.61% 通过。HB-M3 五工具代理与 Settings 开发检查面已交付（Writing MCP 27d29b3 与 Storyforge 959872d 配对记录）：Bearer 门禁的五工具代理路由（envelope 校验 + 新冻结错误码 BRIDGE_TOOL_REQUEST_INVALID、bridge-owned workRef 与 WORK_REF_NOT_FOUND 单次重解析、explore/context/diagnose 15s 与 resolve/index 120s 冻结超时、diagnose 强制 metadata 并移除绝对诊断路径、真实 MCP stdio 五工具 fixture 链）；Storyforge 薄 JSON 客户端 + 内存 token store + WritingBridgePanel（DEV && VITE_WRITING_BRIDGE_ENABLED 门内，初始渲染零网络调用，start_capture 的 diagnosticRunRef 自动带入 finish_capture）+ 跨仓库协议 fixture 漂移门禁（HB-M0 计划锚点补齐）。全链 pnpm verify 52 文件/271 测试、公共基准 30/30 通过；Storyforge 12 项目标测试、eslint 0 警告、tsc 0 错、生产 build 通过。HB-M4 章节生成 preflight 与真实 ContextPacket 注入已交付（Storyforge eca7b3e，Writing MCP 侧无变更）：preflight.ts（末条 user 规范化 query → 快照 → context(4000) → 唯一 mcpEvidence 段、证据 ref 去重、旧段先剥离、targetChapter 仅真实章节且取自快照映射、bridge 失败硬阻断不静默降级、overflow 强制 reject）、AICallMeta.chapterTarget 判别联合 + writingBridge enabled/bypass_once、章节生成节点 preflight 钩子（blocked 不触 provider）、ChapterEditor 接线 + 重试/单次绕过对话框（bypass 一次性不持久化）；12 项先失败测试（Storyforge tests/writing-bridge 6 文件 24 项全绿、eslint 0、tsc 0、生产 build 通过；Writing MCP 不变，52 文件/271 测试先前全绿）。HB-M5 联合启动脚本、HB-M6 真实宿主验收与治理收口均未开始，不得据计划文本宣称 M5 宿主联动完成。
...
| M5 | 进行中（客户端文档纵向切片已完成公共验证链逐项复核） | stdio、五工具注册、structuredContent、outputSchema、统一结果/错误/诊断信封、协议测试、单个真实转换型 EPUB 回归；`CLIENT_SETUP.md` 覆盖 Node 24、pnpm/build、Qoder/Codex stdio 配置、首次调用路由、七类故障、诊断隐私与 v1 边界 | 真实 InkOS、更多 EPUB 2/3 变体、实际客户端安装/连通性验收；Storyforge Host Bridge 真实宿主联动验收（2026-08-29 纳入，2026-08-31 改以计划 `HB-M0`～`HB-M6` milestone 和 `M5_REQUIRED` / `BRIDGE_GOVERNANCE_REQUIRED` 收口；HB-M0 协议冻结 `9048b21`、HB-M1 安全 companion `2148329`、HB-M2 快照事务（Writing MCP `1f70da6` + Storyforge `330d980` 配对）已交付；HB-M3 五工具代理、HB-M4 注入、HB-M5 联合启动、HB-M6 验收均未开始，不得宣称 M5 宿主联动完成） |
...
- 52 个测试文件 / 271 项测试，覆盖：通用链路、MCP 协议与诊断、M5 客户端文档契约、HB-M0 host-bridge 协议与 storyforge manifest 冻结、HB-M1 配对/令牌/安全边界/单实例锁/MCP stdio 客户端/CLI 生命周期、HB-M2 快照事务/binding/派生数据删除与 Storyforge 确定性导出、HB-M3 五工具代理路由/工作引用恢复/脱敏诊断与 Storyforge 薄客户端/Settings 开发面/协议 fixture 门禁、诊断 retention/协作锁、server/core 响应字节上限与 recorder ordering、统一进程关闭链、检索正确性、短原词候选保留、revision-scoped 暖查询复用与索引失效、A1 SourceSnapshot/fingerprint、A2 evaluator/gold/private/corpus 只读门禁、两遍 PRF/双字候选/批量频率上限、公开前隐私门禁、上下文装配、BFS、timeline、图词汇、基准、TXT/EPUB/InkOS、路径安全和索引生命周期。
...
- Host Bridge：从 HB-M3 开始，先以失败测试冻结 resolve/index/explore/context/diagnose 代理、`WORK_REF_NOT_FOUND` 重解析、metadata-only diagnose 与工具超时，再实现 Storyforge 薄客户端和仅开发态可见的 Settings 检查面。
- 公开化收尾：创建 `.github/workflows/verify.yml`（Node 24 / pnpm / fetch-depth: 0 跑 `pnpm verify`，含 privacy:gate）；私有验收链（`verify:private`）永不进 CI。GitHub 服务端旧对象由平台 GC 回收，不承诺即时；如公开前需彻底清除缓存视图可联系 GitHub support。
- 可靠性修复 Task 7 已关闭：完整 public/private 门禁、契约/状态/v2/测试清单/运行 skill 一致性核对及整分支 Critical/Important 复审均已完成；可靠性分支已合并进 `main`。剩余仅为公开前隐私收尾及远端历史对齐。
...
- `eca7b3e` — feat(writing-bridge): HB-M4 章节调用前注入 MCP 证据——preflight 纯函数（query/快照/context 4000/唯一 mcpEvidence 段/去重/旧段剥离/targetChapter 仅真实章节且取快照映射/bridge 失败硬阻断/overflow reject）、AICallMeta.chapterTarget 判别联合、章节生成节点 preflight 钩子、ChapterEditor 接线与重试/单次绕过对话框；12 项先失败测试；Storyforge 侧 24 项 Bridge 测试全绿。Writing MCP 无变更（52 文件/271 测试先前全绿）。本地提交，未推送。
