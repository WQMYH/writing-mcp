...
- Two-character expansion terms share one bounded deterministic LIKE candidate pass with original terms; terms of three or more characters use FTS candidate expansion with deterministic LIKE fallback. Expansion candidates are capped at 256. `writing_context` uses a separate internal search pool of 12 results before layer/dedup/budget assembly; this is an internal performance bound and does not change the public `writing_context` schema or `requiredRefs` direct-resolution guarantee.
