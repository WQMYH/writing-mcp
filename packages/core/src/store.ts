...
export class WritingStore {
  ...
  private searchRows(db:DatabaseSync,query:string,limit:number,experiment?:SearchExperimentOptions,prf=experiment?.prf):{rows:Array<Record<string,unknown>>;diagnostics:ExploreResult["diagnostics"]}{
    ...
    const documentFrequency=(term:string)=>{const key=`${revision}\0${term}`,existing=this.prfDocumentFrequencyCache.get(key);if(existing!==undefined)return existing;let count:number;try{if([...term].length===3){if(!this.prfVocabularyReady){db.exec("CREATE VIRTUAL TABLE IF NOT EXISTS temp.prf_spans_vocab USING fts5vocab(main, spans_fts, 'row')");this.prfVocabularyReady=true;}const row=db.prepare("SELECT doc FROM temp.prf_spans_vocab WHERE term=?").get(term) as Record<string,unknown>|undefined;count=asNumber(row?.doc);}else count=asNumber((db.prepare("SELECT COUNT(*) count FROM spans_fts WHERE spans_fts MATCH ?").get(`"${term}"`) as Record<string,unknown>).count);}catch{const pattern=`%${term.replaceAll("\\","\\\\").replaceAll("%","\\%").replaceAll("_","\\_")}%`;count=asNumber((db.prepare("SELECT COUNT(*) count FROM spans WHERE heading LIKE ? ESCAPE '\\' OR content LIKE ? ESCAPE '\\'").get(pattern,pattern) as Record<string,unknown>).count);}this.prfDocumentFrequencyCache.set(key,count);if(this.prfDocumentFrequencyCache.size>4096)this.prfDocumentFrequencyCache.delete(this.prfDocumentFrequencyCache.keys().next().value!);return count;};
    const termCacheKey=`${revision}\0${query}\0${limit}\0${[...disabled].sort(compareText).join(",")}\0${prf.topK}`;
    let cachedTerms=this.prfTermCache.get(termCacheKey);if(!cachedTerms){cachedTerms=derivePrfTerms(baseline.map(row=>({heading:String(row.heading),excerpt:String(row.content)})),terms,persistedAliases,{...prf,termCount:8},totalSpans,documentFrequency);this.prfTermCache.set(termCacheKey,cachedTerms);if(this.prfTermCache.size>256)this.prfTermCache.delete(this.prfTermCache.keys().next().value!);}
