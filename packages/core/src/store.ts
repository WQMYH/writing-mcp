...
export class WritingStore {
  ...
  async context(query:string,budgetTokens:number,requiredRefs:string[]=[]):Promise<ContextPacket>{if(requiredRefs.length>128||requiredRefs.some(ref=>ref.length>256))throw codedError("CONTEXT_REFS_TOO_LARGE","requiredRefs exceeds 128 items or a 256-character reference");if(!Number.isFinite(budgetTokens)||budgetTokens<1||budgetTokens>1_000_000)throw codedError("BUDGET_OUT_OF_RANGE","budgetTokens must be between 1 and 1000000");const explored=await this.explore("search",query,50,2);const candidates:ContextBlock[]=explored.results.map((r,i)=>({...r,layer:i<3?"L1":i<10?"L2":"L3",tokens:estimateTokens(r.evidence.excerpt),required:requiredRefs.includes(r.ref)}));const poolRefs=new Set(candidates.map(c=>c.ref)),direct:ContextBlock[]=[],unresolved:string[]=[];
    if(requiredRefs.length){const db=await this.open();for(const ref of [...new Set(requiredRefs)]){if(poolRefs.has(ref))continue;const node=this.nodeItem(db,ref,1);if(node)direct.push({...node,layer:"L1",tokens:estimateTokens(node.evidence.excerpt),required:true});else unresolved.push(ref);}}
    const required=[...candidates.filter(c=>c.required),...direct],min=required.reduce((n,c)=>n+c.tokens,0);if(min>budgetTokens)return{status:"budget_unsatisfiable",workRef:this.work.workRef,revision:explored.revision,budgetTokens,usedTokens:0,estimated:true,estimator:"mixed-cjk-v1",blocks:[],omitted:[...candidates.map(c=>({ref:c.ref,reason:"required_minimum_exceeds_budget",tokens:c.tokens})),...direct.map(c=>({ref:c.ref,reason:"required_minimum_exceeds_budget",tokens:c.tokens})),...unresolved.map(ref=>({ref,reason:"required_minimum_exceeds_budget",tokens:0}))],diagnostics:[]};const blocks:ContextBlock[]=[];let used=0;for(const c of [...required,...candidates.filter(c=>!c.required)]){if(blocks.some(b=>b.ref===c.ref))continue;if(used+c.tokens<=budgetTokens){blocks.push(c);used+=c.tokens;}}const omitted=[...candidates.filter(c=>!blocks.some(b=>b.ref===c.ref)).map(c=>({ref:c.ref,reason:"budget_limit",tokens:c.tokens})),...direct.filter(c=>!blocks.some(b=>b.ref===c.ref)).map(c=>({ref:c.ref,reason:"budget_limit",tokens:c.tokens})),...unresolved.map(ref=>({ref,reason:"not_found",tokens:0}))];return{status:omitted.length?"truncated":"complete",workRef:this.work.workRef,revision:explored.revision,budgetTokens,usedTokens:used,estimated:true,estimator:"mixed-cjk-v1",blocks,omitted,diagnostics:explored.diagnostics};}
