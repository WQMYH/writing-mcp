...
export class WritingStore {
  ...
  private searchRows(db:DatabaseSync,query:string,limit:number):{rows:Array<Record<string,unknown>>;diagnostics:ExploreResult["diagnostics"]}{
    const terms=this.analyzeQuery(query);if(!terms.length)return{rows:[],diagnostics:[{code:"NO_MATCHING_TERMS",message:"The query contains no searchable deterministic terms"}]};const aliasTerms=[...new Set(terms.flatMap(term=>/^[\u3400-\u9fff]{2,3}$/.test(term)?[`阿${term.at(-1)}`,`小${term.at(-1)}`,`老${term.at(-1)}`,...(term.length===3?[term.slice(1)]:[])]:[]).filter(alias=>!terms.includes(alias)))],searchTerms=[...terms,...aliasTerms];
  ...
  async context(query:string,budgetTokens:number,requiredRefs:string[]=[]):Promise<ContextPacket>{if(requiredRefs.length>128||requiredRefs.some(ref=>ref.length>256))throw codedError("CONTEXT_REFS_TOO_LARGE","requiredRefs exceeds 128 items or a 256-character reference");if(!Number.isFinite(budgetTokens)||budgetTokens<1||budgetTokens>1_000_000)throw codedError("BUDGET_OUT_OF_RANGE","budgetTokens must be between 1 and 1000000");const explored=await this.explore("search",query,50,2);const candidates:ContextBlock[]=explored.results.map(r=>{const required=requiredRefs.includes(r.ref),profile=contextSourceProfile(r.kind,required);return{...r,layer:profile.layer,tokens:estimateTokens(r.evidence.excerpt),required};});const{kept,duplicates}=dedupByEvidence([...candidates.filter(c=>c.required),...candidates.filter(c=>!c.required)]);const poolRefs=new Set(kept.map(c=>c.ref)),direct:ContextBlock[]=[],unresolved:string[]=[];
    if(requiredRefs.length){const db=await this.open();for(const ref of [...new Set(requiredRefs)]){if(poolRefs.has(ref))continue;const node=this.nodeItem(db,ref,1);if(node)direct.push({...node,layer:"L0",tokens:estimateTokens(node.evidence.excerpt),required:true});else unresolved.push(ref);}}
    const required=[...kept.filter(c=>c.required),...direct],min=required.reduce((n,c)=>n+c.tokens,0);if(min>budgetTokens)return{status:"budget_unsatisfiable",workRef:this.work.workRef,revision:explored.revision,budgetTokens,usedTokens:0,estimated:true,estimator:"mixed-cjk-v1",blocks:[],omitted:[...kept.map(c=>({ref:c.ref,reason:"required_minimum_exceeds_budget",tokens:c.tokens})),...direct.map(c=>({ref:c.ref,reason:"required_minimum_exceeds_budget",tokens:c.tokens})),...duplicates.map(c=>({ref:c.ref,reason:"required_minimum_exceeds_budget",tokens:c.tokens})),...unresolved.map(ref=>({ref,reason:"required_minimum_exceeds_budget",tokens:0}))],diagnostics:[]};const optional=kept.filter(c=>!c.required).sort((a,b)=>layerRank(a.layer)-layerRank(b.layer)||b.score-a.score||contextSourceProfile(a.kind,false).priority-contextSourceProfile(b.kind,false).priority||compareText(a.ref,b.ref));const blocks:ContextBlock[]=[];let used=0;for(const c of [...required,...optional]){if(blocks.some(b=>b.ref===c.ref))continue;if(used+c.tokens<=budgetTokens){blocks.push(c);used+=c.tokens;}}const omitted=[...duplicates.map(c=>({ref:c.ref,reason:"duplicate_evidence",tokens:c.tokens})),...kept.filter(c=>!blocks.some(b=>b.ref===c.ref)).map(c=>({ref:c.ref,reason:"budget_limit",tokens:c.tokens})),...direct.filter(c=>!blocks.some(b=>b.ref===c.ref)).map(c=>({ref:c.ref,reason:"budget_limit",tokens:c.tokens})),...unresolved.map(ref=>({ref,reason:"not_found",tokens:0}))];return{status:omitted.length?"truncated":"complete",workRef:this.work.workRef,revision:explored.revision,budgetTokens,usedTokens:used,estimated:true,estimator:"mixed-cjk-v1",blocks,omitted,diagnostics:explored.diagnostics};}
