...
export class WritingStore {
  ...
  private searchRows(db:DatabaseSync,query:string,limit:number,experiment?:SearchExperimentOptions,prf=experiment?.prf):{rows:Array<Record<string,unknown>>;diagnostics:ExploreResult["diagnostics"]}{
    ...
    const runPass=(expansionTerms:ReadonlyArray<{term:string;weight:number}>=[],passLimit=limit)=>{
      ...
      const addLikeCandidates=(searchTerms:readonly string[])=>{if(!searchTerms.length)return;const escaped=searchTerms.map(term=>`%${term.replaceAll("\\","\\\\").replaceAll("%","\\%").replaceAll("_","\\_")}%`),where=searchTerms.map(()=>"(s.heading LIKE ? ESCAPE '\\' OR s.content LIKE ? ESCAPE '\\')").join(" OR ");for(const row of db.prepare(`${base} WHERE ${where} ORDER BY d.source_ordinal,s.ordinal,s.span_ref LIMIT ?`).all(...escaped.flatMap(value=>[value,value]),candidateLimit) as Array<Record<string,unknown>>)candidates.set(String(row.span_ref),row);};
      addLikeCandidates(originalSearchTerms);addLikeCandidates(expansionTerms.map(item=>item.term));
    ...
    const documentFrequency=(term:string)=>{const key=`${revision}\0${term}`,existing=this.prfDocumentFrequencyCache.get(key);if(existing!==undefined)return existing;let count:number;if([...term].length>=3)try{count=asNumber((db.prepare("SELECT COUNT(*) count FROM spans_fts WHERE spans_fts MATCH ?").get(`"${term}"`) as Record<string,unknown>).count);}catch{const pattern=`%${term.replaceAll("\\","\\\\").replaceAll("%","\\%").replaceAll("_","\\_")}%`;count=asNumber((db.prepare("SELECT COUNT(*) count FROM spans WHERE heading LIKE ? ESCAPE '\\' OR content LIKE ? ESCAPE '\\'").get(pattern,pattern) as Record<string,unknown>).count);}else{const pattern=`%${term.replaceAll("\\","\\\\").replaceAll("%","\\%").replaceAll("_","\\_")}%`;count=asNumber((db.prepare("SELECT COUNT(*) count FROM spans WHERE heading LIKE ? ESCAPE '\\' OR content LIKE ? ESCAPE '\\'").get(pattern,pattern) as Record<string,unknown>).count);}this.prfDocumentFrequencyCache.set(key,count);if(this.prfDocumentFrequencyCache.size>4096)this.prfDocumentFrequencyCache.delete(this.prfDocumentFrequencyCache.keys().next().value!);return count;};
