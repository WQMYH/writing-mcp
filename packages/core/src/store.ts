...
export class WritingStore {
  ...
  private item(r:Record<string,unknown>):ExploreItem{return {ref:String(r.ref??r.span_ref),kind:String(r.kind??"span"),title:String(r.heading),score:Number(r.score??0),sourceKind:"deterministic",confidence:1,evidence:{documentRef:String(r.document_ref??r.span_ref),relativePath:String(r.relative_path),startLine:asNumber(r.start_line),endLine:asNumber(r.end_line),excerpt:String(r.content).slice(0,900)}};}

  private nodeItem(db:DatabaseSync,ref:string,score:number):ExploreItem|undefined{
    const entity=db.prepare("SELECT e.entity_ref ref,e.kind,e.name heading,s.content,s.document_ref,d.relative_path,s.start_line,s.end_line,e.source_kind,e.confidence FROM entities e JOIN spans s ON s.span_ref=e.span_ref JOIN documents d ON d.document_ref=s.document_ref WHERE e.entity_ref=?").get(ref) as Record<string,unknown>|undefined;
    if(entity)return{...this.item({...entity,score}),sourceKind:String(entity.source_kind) as ExploreItem["sourceKind"],confidence:Number(entity.confidence)};
    ...
    if(span)return this.item({...span,score});
    const document=db.prepare("SELECT d.document_ref ref,d.kind,d.title heading,s.content,d.document_ref,d.relative_path,s.start_line,s.end_line FROM documents d JOIN spans s ON s.document_ref=d.document_ref WHERE d.document_ref=? ORDER BY s.ordinal LIMIT 1").get(ref) as Record<string,unknown>|undefined;
    return document?this.item({...document,score}):undefined;
  }
