...
export class WritingStore {
  ...
  private searchRows(db:DatabaseSync,query:string,limit:number,experiment?:SearchExperimentOptions):{rows:Array<Record<string,unknown>>;diagnostics:ExploreResult["diagnostics"]}{
    const terms=this.analyzeQuery(query);if(!terms.length)return{rows:[],diagnostics:[{code:"NO_MATCHING_TERMS",message:"The query contains no searchable deterministic terms"}]};const disabled=new Set(experiment?.disabledFactors??[]),aliasTerms=disabled.has("alias")?[]:[...new Set(terms.flatMap(term=>/^[\u3400-\u9fff]{2,3}$/.test(term)?[`阿${term.at(-1)}`,`小${term.at(-1)}`,`老${term.at(-1)}`,...(term.length===3?[term.slice(1)]:[])]:[]).filter(alias=>!terms.includes(alias)))],searchTerms=[...terms,...aliasTerms];
