...
| `host-bridge-mcp-client.test.ts` | HB-M1 MCP stdio 客户端：initialize 握手 + 真实 writing_resolve 调用（信封 structuredContent.result.data.status）、stderr 有界且确实保留最新尾部（含溢出后的末尾 marker）、stdout 非 JSON 噪声不影响协议通道、子进程中途退出拒绝挂起请求为 BRIDGE_MCP_UNAVAILABLE、stop 幂等且子进程终止 |
