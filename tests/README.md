...
| `host-bridge-snapshot.test.ts` | HB-M2 快照事务：激活写 source + resolve(adapterHint=generic)→index(status→incremental/rebuild 分支)→fresh binding 七字段 manifest、同快照 fresh 时 no-op 零 MCP 调用、伪造文档哈希拒绝而 claimedSnapshotHash 忽略、16 MiB/64 MiB/4096 文档限额、逃逸路径不入盘、EBUSY 有界重试、索引不 fresh 回滚旧 source 且 binding 保持 fresh、恢复失败转 degraded、重启后 manifest 装载为 stale 需重激活、禁用/撤销拒绝新快照但不删派生数据、路由与 draft 的 hostProjectId 不一致拒绝 |
